...
/*	global $Team;
	...
	if (file_exists($fileN)) {
		...
		$tBC_ln = $Team[$nat]["Long"];
		$_SESSION['T'] = $Team;
		$map_link = "<a href=\"map_page.php?lat=".$tBC_lt."&lng=".$tBC_ln."&z=".$tBC_z."&t=t".$c."&n=".$n."\" target=\"_new\"><i class=\"material-icons\">map</i></a>";
		...
		foreach ($ladder as $t) {
			...
			if ($p[0] == "group") {
				...
				$goals = "<td class=\"".$style."\">".$p[4]."</td><td class=\"".$style."\">".$p[5]."</td><td class=\"".$style."\">".$gd."</td>";
				$pts_fate = "<td class=\"".$style."\"><b>".$p[6]."</b></td><td class=\"text-center ".$style."\">".$fate."</td>";
				print t(8)."<tr>".$team.$games.$goals.$pts_fate."</tr>\n";
			}
		}
		print t(8)."</tbody></table>\n";
		print t(7)."</div>\n";
	}
	else {
